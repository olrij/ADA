...
-- Actualiza la tabla anidada de personas 
-- asociadas al grupo “DIRECCIÓN” para añadir una nueva persona.
INSERT INTO TABLE (SELECT PERSONA_ANIDADA FROM GRUPOS2 GP WHERE GP.NOMBRE='DIRECCIÓN') VALUES (PERSONA(100, 'Manuel', Direccion('c/ Cuba','Málaga',10001),'27/02/1989'));
-- Actualiza la tabla anidada de personas asociadas al grupo “DIRECCIÓN” 
-- para eliminar a la persona con código 8.
-- EN vez de borrar el 
DELETE FROM TABLE (SELECT PERSONA_ANIDADA FROM GRUPOS2 GP WHERE GP.NOMBRE='ADMINISTRACIÓN') ADM WHERE ADM.CODIGO=8;
-- Modifica la fecha de creación del grupo con id 2 para que sea ‘20/05/2021’.
UPDATE GRUPOS2 SET FECHA_CREACION='20/05/2021' WHERE ID=2;
-- Borra el grupo INFORMÁTICA.
DELETE FROM GRUPOS2 WHERE NOMBRE='INFORMÁTICA';
